...
namespace ZKube.Integration.Execution
{
    ...
    {
        private static bool SupportsRun(string name) => name == "enter_arena" ||
        ...
        private async Task<bool> ReconcileRun(ExecutionReconciliation evidence, DecodedProtocolInstruction[] instructions, CancellationToken cancellation)
        {
            ...
            if (claims > 0)
            {
                if (claims > PlanningConstants.MaxAutoClaims || claims == instructions.Length ||
                    ...
                    instructions.Take(claims).Any(i => i.Accounts["owner_authority"] != evidence.Pending.Owner ||
                        i.Accounts["player_state"] != planner.Player(evidence.Pending.Owner))) return false;
                instructions = instructions.Skip(claims).ToArray();
            }
            if (instructions.Length == 0 || instructions.Any(i => !SupportsRun(i.Name))) return false;
            ...
            var observation = Observed(evidence, address);
            ...
            if (consume != null)
            {
                ...
                {
                    if ((current != 0 && planner.ActiveRun(owner, current) == address) || (await rpc.Placement(address)).IsDelegated) return false;
                    await acceptRun(new RunRecordChange(owner, address, null, RunRecordState.Consumed, playerInfo.Envelope));
                    return true;
                }
                if (succeeded) return false;
                return await AcceptRun(observation, owner, address, instructions, false);
            }
            ...
            if (prepare != null)
            {
                if (!evidence.Pending.IsBase || planner.ActiveRun(owner, expectedRun.Value) != address)
                    throw new FormatException("Preparation does not match its Base run PDA");
                var playerInfo = Observed(evidence, prepare.Accounts["player_state"]);
                if (playerInfo.Envelope == null) return false;
                var player = accounts.PlayerState(playerInfo.Envelope, owner);
                ...
                if (!succeeded && observation.Envelope == null)
                {
                    ...
                    await acceptRun(new RunRecordChange(owner, address, expectedRun, RunRecordState.Absent));
                    return true;
                }
            }
            ...
            return await AcceptRun(observation, owner, address, instructions, succeeded, expectedRun);
        }

        private async Task<bool> AcceptRun(RpcAccount observation, string owner, string address,
            DecodedProtocolInstruction[] instructions, bool succeeded, ulong? expectedRun = null)
        {
            ...
            if (address != planner.ActiveRun(owner, runId) || (expectedRun.HasValue && runId != expectedRun.Value)) throw new FormatException("Observed run does not match signed intent");
            if (succeeded)
            {
                foreach (var instruction in instructions)
                {
            ...
            await acceptRun(new RunRecordChange(owner, address, runId, RunRecordState.Present));
            return true;
        }
        ...
        {
            if ((await rpc.Placement(address)).IsDelegated) return false;
            var batch = await rpc.ReadAccounts(rpc.Base, new[] { address, planner.Player(evidence.Pending.Owner) },
            ...
            foreach (string field in new[] { "active_run_id" })
            {
                ...
                if (current != 0 && planner.ActiveRun(evidence.Pending.Owner, current) == address) return false;
            }
            ...
            await acceptRun(new RunRecordChange(evidence.Pending.Owner, address, runId, RunRecordState.Consumed, batch.Accounts[1].Envelope));
            return true;
        }
    }
}

...
namespace ZKube.Integration.Tests
{
    public sealed class ApplicationReconciliationTests
    {
            ...
            private async Task<JToken> Respond(Uri endpoint, JObject request, CancellationToken cancellation)
            {
                ...
                switch ((string)request["method"])
                {
                    ...
                    case "getMultipleAccounts":
                        result = TestHttp.Context(new JArray(request["params"][0].Values<string>().Select(address => Accounts.TryGetValue(address, out var value) ? value.DeepClone() : JValue.CreateNull())), AccountSlot); break;
        ...
        public async Task SignedRenewalResumesPublicSaveBeforeJournalRemoval()
        {
            ...
            {
                var store = new TestMemory(); var native = new TestNative { Seed = Enumerable.Repeat((byte)2, 32).ToArray(), ForbidRequests = true, ForbidKeyCreation = true }; var records = new SessionRecordStore(store, tokens, protocol.ProgramId);
        ...
        public async Task PurchaseValidatesFreshAccountsAndAwaitsInvalidationBeforeClearingJournal()
        {
            ...
            var executor = new TransactionExecutor(planner, rpc, new WalletClient(new TestNative { Seed = Enumerable.Repeat((byte)2, 32).ToArray(), ForbidRequests = true, ForbidKeyCreation = true }), journal);

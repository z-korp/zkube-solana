#!/usr/bin/env python3
"""Reproduce local Unity art from the existing client sources and pinned fonts.
...
ROOT = PROJECT.parent
SOURCE = ROOT / "client/public/assets"
...
def theme_catalog():
    env = dict(os.environ, NO_DNA="1", TSX_TSCONFIG_PATH="tsconfig.app.json")
    result = subprocess.run(["node", "--import", "tsx", "tools/unity/theme-fixtures.ts"],
                            cwd=ROOT / "client", env=env, check=True, capture_output=True)
...
def plan():
    ...
    by_source = {"/assets/" + Path(e["source"]).relative_to("client/public/assets").as_posix(): e for e in entries}
    ...
    catalog.update(assets=entries, fonts=font_entries,
        ...
        importPolicy={"schema": 1, "source": "client/public/assets", "pixelsPerUnit": 100,

...
const fixtures = vi.hoisted(() => ({
  ...
  directRunHook: vi.fn(() => {
    throw new Error("useRebootDaily created a second run controller");
  }),
  run: {
...
vi.mock("@/solana/connectionContext", () => ({
  useSolanaConnection: () => ({ connection: fixtures.connection }),
}));

vi.mock("@/solana/reboot/dailyClient", () => ({
...
vi.mock("@/solana/reboot/paymasterClient", () => ({
  fetchPaymasterClient: vi.fn(),
}));

vi.mock("@/solana/reboot/runPlan", () => ({
  submitSponsoredTransactionPlan: vi.fn(),
}));

vi.mock("@/solana/reboot/useRebootRun", () => ({
  useRebootRun: fixtures.directRunHook,
}));

describe("useRebootDaily run sharing", () => {
  beforeEach(() => {
    vi.clearAllMocks();
  ...
  it("uses the RunProvider controller without creating a second controller", async () => {
    const { result } = renderHook(() => useRebootDaily());

...
test("encodes independent slots and invokes actual native TS decoding and recovery", async () => {
  ...
  try {
    ...
    expect(new Set(fixture.cases.map(row => row.address)).size).toBe(2);
    expect(fixture.routing.every(row => row.phase === "delegated")).toBe(true);
    expect(fixture.sessionDecisions).toHaveLength(10);
    expect(fixture.sessionDecisions.every(row => row.phase === "settleable")).toBe(true);
    expect(Object.keys(fixture.deviceConsume)).toEqual(["campaign", "daily"]);

...
import { canonicalCampaignMap, CAMPAIGN_CONTENT_VERSION, CANONICAL_CAMPAIGN_MAP_COUNT } from "../../src/core/campaignCatalog";
...
import { DAILY_REWARD_CLAIM_WINDOW_SECONDS, DAILY_MAX_MOVES, ARCADE_ACCOUNT_VERSION, PROTOCOL_ACCOUNT_VERSION } from "../../src/core/protocolVersions.generated";
...
export async function generateProductReadFixtures() {
  ...
  const catalogs: Envelope[]=[];
  for(let map=1;map<=CANONICAL_CAMPAIGN_MAP_COUNT;map++) {
    const catalog=canonicalCampaignMap(CAMPAIGN_CONTENT_VERSION,map);
    catalogs.push(put(await encode("mapCatalog",pdas.deriveMapCatalogPda(CAMPAIGN_CONTENT_VERSION,map),{
      ...catalog,version:PROTOCOL_ACCOUNT_VERSION,contentVersion:CAMPAIGN_CONTENT_VERSION,bump:0
    })));
  }
  ...
  const badCatalogFields=coder.decode<Record<string,unknown>>("mapCatalog",Buffer.from(catalogs[0]!.data,"base64"));
  const invalidAccounts={
    ...
    dailyContentVersion:await encode("arenaDaily",pdas.deriveArenaDailyPda(day),{...dailyFields,
      contentVersion:0}),
    ...
    catalogMap:await encode("mapCatalog",new PublicKey(catalogs[0]!.address),{...badCatalogFields,mapId:2}),
    catalogVersion:await encode("mapCatalog",new PublicKey(catalogs[0]!.address),{...badCatalogFields,version:255}),
  ...
  const active=runs.cases.find((row:{id:string})=>row.id==="active-campaign-playing") as Envelope;
  ...
  return {schemaVersion:1,inputs:{now,day,oldDay,owner:owner.toBase58()},accounts:{...plans.accounts,player,daily,oldDaily,catalogs,active},
    ...
    dailyAuthority:{day,pairIndex:pair.pairIndex,contentVersion:CAMPAIGN_CONTENT_VERSION,maxMoves:DAILY_MAX_MOVES,

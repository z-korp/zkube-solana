...
pub struct RequestRowVrfV1<'info> {
    ...
    /// CHECK: Address-constrained to the delegation record for active_run.
    #[account(address = ephemeral_rollups_sdk::pda::delegation_record_pda_from_delegated_account(&active_run.key().to_bytes().into()).to_bytes().into())]
...
pub fn handler_request_row_vrf_v1(
    ...
    let validator = {
        let data = ctx.accounts.delegation_record_active.try_borrow_data()?;
        require!(data.len() >= 40, ErrorCode::InvalidMagicProgram);
        Pubkey::try_from(&data[8..40]).map_err(|_| error!(ErrorCode::InvalidMagicProgram))?
    };

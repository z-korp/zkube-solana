...
export default function RebootSpectatorScreen() {
  const navigate = useNavigationStore((state) => state.navigate);
  const rawTarget = useNavigationStore((state) => state.spectateTarget);

  const parsed = useMemo<{ target: SpectateTarget | null; error: string | null }>(() => {
  ...
  const gridSize = Math.max(20, Math.min(52, Math.floor((boardHeight - 90) / 11)));
  ...
  const watchedLabel = rawTarget?.player
    ? shortKey(rawTarget.player)
    : rawTarget?.pda
      ? shortKey(rawTarget.pda)
      : "—";

...
use zkube_core::{
    ARENA_ENTRY_LAMPORTS, Bonus, CAMPAIGN_TARGET_LADDER, Constraint, ConstraintKind,
    DAILY_MAX_MOVES, DAILY_PAIR_COUNT, DAILY_REWARD_CLAIM_WINDOW_SECONDS, ENTRY_DAILY_LAMPORTS,
    ENTRY_OPERATOR_LAMPORTS, Guardian, PLAYER_STATE_ACCOUNT_VERSION, PRESSURE_STEP,
    PROTOCOL_ACCOUNT_VERSION, RunRules, SECONDS_PER_DAY, SOL_PAYOUT_UNIT_LAMPORTS,
};

const FIXTURE: &str = "fixtures/campaign-catalog.json";
...
fn validate_catalog(catalog: &CampaignCatalog) -> Result<(), String> {
    if catalog.schema_version != 1 || catalog.content_version != zkube_core::CATALOG_VERSION {
        return Err(format!(
            "Campaign catalog requires schemaVersion 1 and contentVersion {}",
            zkube_core::CATALOG_VERSION
        ));
    }
...
fn render_realm_rules_rust(catalog: &CampaignCatalog) -> String {
    ...
    for map in &catalog.maps {
        ...
        let bonus = format!(
            "{:?}",
            Bonus::from_tag(u8::try_from(bonus).unwrap()).unwrap()
        );
...
fn campaign_rules(
    ...
    let bonus = u8::try_from(map.rules[0])
        .ok()
        .and_then(Bonus::from_tag)
        .ok_or_else(|| format!("map {} has unknown bonus {}", map.map_id, map.rules[0]))?;
    RunRules::campaign(

...
use zkube_core::{
    ARCADE_ACCOUNT_VERSION, Bonus, CampaignRules, Constraint, ConstraintKind, EndlessRules,
    LevelRules, MutatorRules, PLAYER_LABEL_ACCOUNT_VERSION, PROTOCOL_ACCOUNT_VERSION,
    RULES_ACCOUNT_VERSION, Sha256Provider, SoftwareSha256,
};

const FIXTURE: &str = "fixtures/campaign-v2.json";
const GENERATED_TS: &str = "crates/zkube-codegen/generated/catalog.generated.ts";
const GENERATED_VERSION_TS: [&str; 2] = [
...
fn run(cli: &Cli) -> Result<String, String> {
    ...
    let generated_versions = render_protocol_versions();
    ...
    match &cli.command {
        Command::Generate => {
            ...
            for relative in GENERATED_VERSION_TS {
                let version_output = cli.root.join(relative);
                if let Some(parent) = version_output.parent() {
                    fs::create_dir_all(parent)
                        .map_err(|error| format!("cannot create {}: {error}", parent.display()))?;
                }
                fs::write(&version_output, &generated_versions).map_err(|error| {
                    format!("cannot write {}: {error}", version_output.display())
                })?;
            }
            Ok("generated Campaign catalog and shared protocol versions".into())
        }
        Command::Check => {
            ...
            for relative in GENERATED_VERSION_TS {
                let version_output = cli.root.join(relative);
                let actual = fs::read_to_string(&version_output).map_err(|error| {
                    format!("cannot read {}: {error}", version_output.display())
                })?;
                if actual != generated_versions {
                    return Err(format!(
                        "{} is stale; run `NO_DNA=1 cargo run -p zkube-codegen -- generate`",
            ...
            Ok("checked Campaign catalog and shared protocol versions".into())
        }
    }
}
...
fn render_protocol_versions() -> String {
    format!(
...
mod tests {
    ...
    fn committed_catalog_validates_and_hashes_stably() {
        ...
        let versions = render_protocol_versions();

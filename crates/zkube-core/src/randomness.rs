...
/// verified VRF result after an action clears the board completely.
///
/// Keeping both rows in one domain-separated draw stream avoids the stranded
/// `AwaitingVrf` state that would otherwise require a second oracle request
/// before the player could make another move.
pub type ContinuationLayout = OpeningLayout;
...
impl<H: Sha256Provider> DrawStream<H> {
    fn new(
        domain: &[u8],
        randomness: [u8; 32],
        request_counter: u32,
        ...
        let root = H::hashv(&[domain, &randomness, &request, &rules_hash]);
        Self {
            root,
            block: [0; 32],
            block_index: 0,
    ...
    fn next_u32(&mut self) -> u32 {
        if self.word_index == 8 {
            ...
            self.block = H::hashv(&[b"zkube-draw-block-v1", &self.root, &index]);
...
pub fn opening_from_vrf(
    ...
    opening_from_vrf_with::<SoftwareSha256>(
        randomness,
        request_counter,
        rules_hash,
        height,
...
pub fn continuation_from_vrf_with<H: Sha256Provider>(
    ...
    weights.validate()?;
    let mut stream = DrawStream::<H>::new(
        b"zkube-empty-continuation-v1",
        randomness,
        request_counter,
        rules_hash,
    );
    let seed = packed_row(&mut stream, weights);
    let mut grid = Grid::EMPTY;
    grid.insert_bottom_row(seed)
        .map_err(RandomnessError::InvalidGeneratedRow)?;
    // Generated rows always contain a hole, so settling is gravity-only and
...
pub fn row_from_vrf_with<H: Sha256Provider>(
    ...
    weights.validate()?;
    let mut stream =
        DrawStream::<H>::new(b"zkube-next-row-v3", randomness, request_counter, [0; 32]);
...
fn packed_row<H: Sha256Provider>(stream: &mut DrawStream<H>, weights: BlockWeights) -> Row {
...
fn shuffled_row<H: Sha256Provider>(stream: &mut DrawStream<H>, row: Row) -> Row {
...
mod tests {
    ...
    fn decode_row(value: &Value) -> Row {
        value
            ...
            .unwrap()
            .iter()
            .map(|cell| u8::try_from(cell.as_u64().unwrap()).unwrap())
    ...
    #[test]
    fn perfect_clear_continuation_is_playable_after_one_vrf() {
        let fixture: Value = serde_json::from_str(include_str!(
            "../../../fixtures/replays/golden-perfect-clear-continuation-v1.json"
        ))
        .unwrap();
        assert_eq!(fixture["schema_version"], 1);
        assert_eq!(fixture["domain"], "zkube-empty-continuation-v1");

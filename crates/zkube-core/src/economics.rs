...
pub(crate) const ENTRY_DAILY_BPS: u16 = 9_000;
pub(crate) const ENTRY_OPERATOR_BPS: u16 = 1_000;
...
impl EntrySplit {
    ...
    /// # Errors
    ///
    /// Returns [`EntrySplitError::Overflow`] if the two destinations cannot
...
pub fn split_arena_entry(lamports: u64) -> Result<EntrySplit, EntrySplitError> {
    ...
    let total_bps = u32::from(ENTRY_DAILY_BPS)
        .checked_add(u32::from(ENTRY_OPERATOR_BPS))
        .ok_or(EntrySplitError::Overflow)?;
...
mod tests {
    ...
    #[test]
    fn exact_entry_routes_to_following_daily_and_operator() {
        let split = split_arena_entry(ARENA_ENTRY_LAMPORTS).unwrap();
        assert_eq!(
            split,
            EntrySplit {
                next_daily_lamports: 9_000_000,
